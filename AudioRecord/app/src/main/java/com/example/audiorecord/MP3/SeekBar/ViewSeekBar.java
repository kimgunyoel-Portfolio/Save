package com.example.audiorecord.MP3.SeekBar;
...
import com.example.audiorecord.MP3.Thread.ProgressUpdate;
...
public class ViewSeekBar {
   ...
   private ArrayList<MListViewItem> list;
    ...
    public ViewSeekBar(AppCompatActivity appCompatActivity, MediaPlayer mediaPlayer, int position, ArrayList<MListViewItem> listitem){
        ...
        cProgressUpdate = new ProgressUpdate(aSeekBar, mediaPlayer, seekBar);   //!< SeekBar Thread 객체생성
        cProgressUpdate.start();                                                //!< 쓰레드 시작

        playMusic(list.get(positionbox));                                       //!< 플레이뮤직 메서드
